...
class AccessCodeController extends ResourceController {
  ...
  Map<String, APIResponse> documentOperationResponses(
    ...
    if (operation.method == 'GET') {
      if (operation.pathVariables.contains('role')) {
        return {
          '200': APIResponse.schema('Get access codes by role', context.schema['AccessCode'])
        };
      } else {
        return {'200': APIResponse.schema('Get all access codes', context.schema['AccessCode'])};
      }
    } else if (operation.method == 'POST') {
      return {'200': APIResponse.schema('Add access codes', context.schema['AccessCode'])};
    }
  ...
  Future<Response> postAccessCode() async {
    ...
    final accessCode = AccessCode()..read(await request.body.decode(), ignore: ['id']);
  ...
  @Scope(['admin'])
  @Operation.put('id')
  Future<Response> updateAccesscode(
      @Bind.path('id') int id, @Bind.body(ignore: ['id', 'code']) AccessCode accesscode) async {
